...
- ## RLS
	...
	- Locked down to the authenticated user, scoped to board **membership** (migration `20260629000001_lock_down_rls`) — [[Tables]]
		...
		- `boards_delete_member` (migration `20260629000001_lock_down_rls`) checks `is_board_member(id)`, so any invited collaborator — not just the creator — can delete the whole board. Accepted for now (invited users are trusted), but should be tightened to `owns_board(id)` if boards are ever shared with less-trusted collaborators — tracked in [[Open Work]]
...
- ## Migration filenames must sort after the last applied version
	- Applying a migration by hand during development (MCP `apply_migration`, the SQL editor) records it in the remote history under a **timestamp of that moment**. If the file committed to `supabase/migrations/` then carries an *earlier* version — a round `…000000` stamp, say — `supabase db push` refuses it: it will not insert a migration that sorts before the last one already applied, and the GitHub deploy fails on that push **and every push after it**.
	- Name the file with the version the manual apply actually recorded (`list_migrations` shows it). `20260721143750_card_scheduling_history.sql` is the worked example — it looks like an untidy stamp and is deliberately not a round number.
	- Either apply by hand *and* match the filename to it, or don't apply by hand at all and let the push do it. The failure mode is silent until you look at the Actions run.
