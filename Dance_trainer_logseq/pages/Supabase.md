...
- ## RLS
	...
	- Locked down to the authenticated user, scoped to board **membership** (migration `20260629000001_lock_down_rls`) — [[Tables]]
		...
		- TODO: `boards_delete_member` (migration `20260629000001_lock_down_rls`) checks `is_board_member(id)`, so any invited collaborator — not just the creator — can delete the whole board. Accepted for now (invited users are trusted), but should be tightened to `owns_board(id)` if boards are ever shared with less-trusted collaborators
